...
export default function UploadExcelPage() {
  ...
  const rowsPerPage = 10
    ...
    // 🔹 Tambahkan state baru untuk Size
  const sizeOptions = [
    { value: '2XL', label: '2XL' },
    { value: 'XL', label: 'XL' },
    { value: 'L', label: 'L' },
  ...
  const [selectedSizes, setSelectedSizes] = useState<{ [rowIdx: number]: string }>({})
  

  const handleSizeChange = (rowIdx: number, value: string) => {
    setSelectedSizes((prev) => ({ ...prev, [rowIdx]: value }))
  }
                  ...
                  return (
                    <tr key={rowIdx} className={rowIdx % 2 === 0 ? 'bg-gray-50 dark:bg-gray-800' : 'bg-white dark:bg-gray-900'}>
                      ...
                      <td>
                        <ReactSelect
                          options={sizeOptions}
                          value={sizeOptions.find(opt => opt.value === selectedSizes[globalRowIdx]) || null}
                          onChange={(selected) => {
                            const value = (selected as { value: string; label: string } | null)?.value || '';
                            handleSizeChange(globalRowIdx, value);
                          }}
                          styles={reactSelectStyles}
                        />
                      </td>
