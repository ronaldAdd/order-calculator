...
export default function DebtorForm({ initialData = {}, onSubmit }: Props) {
  ...
  const toReactSelectOptions = (options: string[]): SelectOption[] =>
            ...
            <ReactSelect
              ...
              onChange={(option) => {
                if (!Array.isArray(option)) {
                  ...
                  setForm(prev => ({ ...prev, assignedCollector: singleOption?.value || '' }))
                }
              }}
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <ReactSelect
                ...
                value={productNameOptions.find(opt => opt.value === form.productName) || null}
                onChange={(option) => {
                  if (!Array.isArray(option)) {
        ...
        return (
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <ReactSelect
                options={toReactSelectOptions(sexOptions)}
                value={sexOptions ? { value: form.sex, label: form.sex } : null}
                onChange={(option) => {
                  if (!Array.isArray(option)) {
                    const selected = option as SingleValue<SelectOption>
                    setForm(prev => ({
            ...
            <div>
              <Input
                ...
                inputClass={inputClass}
              />
              {nationalIdError && <p className="text-red-600 mt-1">{nationalIdError}</p>}
            </div>

            <div>
              <label className="block mb-1 font-medium text-gray-700 dark:text-gray-300">Religion</label>
              <ReactSelect
                options={toReactSelectOptions(religionOptions)}
            ...
            <div>
              <label className="block mb-1 font-medium text-gray-700 dark:text-gray-300">Marital Status</label>
              <ReactSelect
                options={toReactSelectOptions(maritalStatusOptions)}
                value={form.maritalStatus ? { value: form.maritalStatus, label: form.maritalStatus } : null}
                onChange={(option) => {
                  if (!Array.isArray(option)) {
                    const selected = option as SingleValue<SelectOption>
                    setForm(prev => ({
        ...
        return (
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <ReactSelect
                options={toReactSelectOptions(statusOptions)}
            ...
            <div>
              <label className="block mb-1 font-medium text-gray-700 dark:text-gray-300">Forecast</label>
              <ReactSelect
                options={toReactSelectOptions(forecastOptions)}
                value={form.forecast ? { value: form.forecast, label: form.forecast } : null}
                onChange={(option) => {
                  if (!Array.isArray(option)) {
                    const selected = option as SingleValue<SelectOption>
                    setForm(prev => ({
            ...
            <div className="md:col-span-2">
              <label htmlFor="nextFollowUpDate" className="block font-medium text-gray-700 dark:text-gray-300">
                Next Follow-Up Date {form.status === 'Promise to Pay' && <span className="text-red-600">*</span>}
              </label>
              <input
                type="date"
                id="nextFollowUpDate"
              ...
              />
              {nextFollowUpDateError && <p className="text-red-600">{nextFollowUpDateError}</p>}
            </div>
          </div>
        )
      default:
